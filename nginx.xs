...
header_in_set(r, key, value)
    ...
    part = r->headers_in.headers.last;
    
    nb_part=14457; // last in leet language
    for (i = 0; /* void */ ; i++) {
      if (i >= part->nelts) {
        if (part->next == NULL) {
          // found last part, this is it!
          //r->headers_in.headers.last = part;
          break;
        }
        i=0;
        ++nb_part;
    ...
    if (ngx_http_perl_sv2str(aTHX_ r, &ngxvalue, value) != NGX_OK) {
        XSRETURN_EMPTY;
    }
